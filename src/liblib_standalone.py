...
使用示例：
    ...
    # 图生图
# TODO: 手动修复长行 -     python liblib_standalone.py --prompt "改成卡通风格" --input-image ./input.jpg --output-dir ./output
"""
...
try:
    ...
    from services.image.liblib_service import (
        AdditionalNetwork,
        F1GenerationParams,
        HiResFixInfo,
        LiblibConfig,
            ...
            if use_f1:
                ...
                if "lora_model_id" in kwargs or "lora_weight" in kwargs:
                    ...
                    if lora_model_id:
                        params.additional_network = [
                            AdditionalNetwork(
                                model_id=lora_model_id, weight=lora_weight
                            )
                        ]
                    else:
                        params.additional_network = []
                ...
                if any(
                    ...
                    if "hires_denoising_strength" in kwargs:
                        params.hi_res_fix_info.hires_denoising_strength = kwargs[
                            "hires_denoising_strength"
                        ]
                            ...
                            for i, image_data in enumerate(status_result.images):
                                ...
                                if image_url:
                                    ...
                                    if output_filename:
                                        ...
                                        filename = (
                                            f"liblib_{result.generate_uuid}_{i+1}.png"
                                        )
...
def batch_generate_from_json(
    ...
    try:
        ...
        if isinstance(data, dict) and "storyboards" in data:
            ...
            for item in data["storyboards"]:
                if isinstance(item, dict):
                    ...
                    if prompt:
                        prompts.append({"prompt": prompt, "original_data": item})
            ...
            for item in data:
                if isinstance(item, dict):
                    ...
                    prompt = item.get(
                        "english_prompt",
                        item.get("故事板提示词", item.get("prompt", "")),
                    )
                    if prompt:
                        prompts.append({"prompt": prompt, "original_data": item})
            ...
            print("支持的格式:")
            print(
                "1. 新标准化格式: 包含'storyboards'数组，每个元素有'english_prompt'字段"
            )
        ...
        for i, prompt_data in tqdm(
            enumerate(prompts, 1), total=total_count, desc="正在生成图片"
        ):
...
使用示例:
  ...
  %(prog)s --prompt "改成卡通风格" --input-image ./input.jpg --output-dir ./output
    ...
    # 模型选项
    parser.add_argument(
        "--use-f1", action="store_true", help="使用F.1模型（默认使用传统模型）"
    )
    ...
    parser.add_argument(
        "--hires-denoising-strength", type=float, help="高分辨率去噪强度"
    )
    ...
    # 并发控制
    parser.add_argument(
        "--max-concurrent", type=int, default=3, help="最大并发数量（默认3）"
    )
    ...
    if args.json_file:
        ...
        batch_generate_from_json(
            service, args.json_file, args.output_dir, args.use_f1, args.max_concurrent
        )
    else:
