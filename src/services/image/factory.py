...
class ImageServiceFactory:
    ...
    def register_service(
        self, service_type: ImageServiceType, service_class: Type[ImageServiceBase]
    ) -> None:
        """
        注册服务类
    ...
    def create_service(
        self, service_type: ImageServiceType, **kwargs
    ) -> Optional[ImageServiceBase]:
        """
        创建服务实例
    ...
    async def get_available_services(self) -> List[ImageServiceBase]:
        ...
        for service in self._service_instances.values():
            try:
                ...
                self.logger.warning(
                    f"检查服务可用性失败 {service.service_type.value}: {str(e)}"
                )
    ...
    async def get_all_service_status(self) -> List[ServiceStatus]:
        ...
        for service in self._service_instances.values():
            try:
                ...
                self.logger.error(
                    f"获取服务状态失败 {service.service_type.value}: {str(e)}"
                )
