...
def process_all_fields_with_model(data_list, character_mappings, story_content):
    ...
    for i in range(0, len(data_list), batch_size):
        ...
        batch_result = process_batch_with_model(
            batch, character_mappings, story_content
        )
...
输入： 读取输入数据中每一个对象的 "narration" 字段。

映射依据： 使用提供的角色映射配置（如下所示）作为唯一标准。

# TODO: 手动修复长行 - 操作： 扫描 "narration" 文本，识别并定位所有出现的角色称谓。将识别到的所有角色称谓（如"你"、"小猪"、"皮皮"、"大灰狼"、"兄弟们"、"生物们"等），严格按照映射关系替换为对应的 "new_name" 值。
...
# TODO: 手动修复长行 - 操作： 判断该句文本所描述的核心事件或环境（例如："建房子"、"发现诅咒"、"被围攻"、"完成仪式"）。用一个简洁的名词性短语（如："森林建设场景"、"诅咒揭示场景"、"村庄围攻场景"、"月光仪式场景"）总结该场景。
...
请直接返回处理后的完整JSON数组，不要添加任何其他说明文字。
    ...
    try:
        ...
        cleaned_response = re.sub(
            r"[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]", "", cleaned_response
        )
...
def create_initial_data_list(sentences):
    ...
    for sentence in sentences:
        ...
        if (
            sentence_clean
            and not is_english_template(sentence_clean)
            and len(sentence_clean) > 3
        ):  # 至少3个字符
            storyboards.append(
                {
                    "scene_id": str(scene_id),
                    "narration": sentence_clean,
                    "english_prompt": "",
                    "processed_chinese": "",
                    "lora_id": "",
                }
...
def process_single_chapter_json(chapter, output_file_path):
    ...
    try:
        ...
        processed_storyboards = process_all_fields_with_model(
            initial_storyboards, character_mappings, content
        )
        ...
        if story_bg:
            ...
            for item in processed_storyboards:
                if item.get("english_prompt"):
                    item["english_prompt"] = (
                        f"{item['english_prompt']}, {translated_bg}"
                    )
...
def process_input_file_directly():
    ...
    try:
        ...
        # 创建一个虚拟章节对象，包含整个文件内容
        chapter = {"title": "完整故事", "content": content}
