            ...
            if status == 'success':
                ...
                for node_id, node_output in outputs.items():
                    ...
                    if isinstance(node_output, dict) and 'video_url' in node_output:
                        ...
                        logger.info(f"从节点{node_id}输出中发现Kling视频URL: {node_output['video_url']}")
                        break
                    # 检查是否有嵌套的结构包含video_url
                        ...
                        for item in node_output:
                            if isinstance(item, dict) and 'video_url' in item:
                                ...
                                logger.info(f"从节点{node_id}输出列表中发现Kling视频URL: {item['video_url']}")
                                break
...
class ImageToVideoGenerator:
    ...
    def __init__(self):
        ...
        # 初始化ComfyUI客户端
        self.client = ComfyUIClient(self.comfyui_server, self.comfyui_api_key)
    ...
    def update_workflow_for_image(self, workflow: Dict, image_path: Path, prompt: str) -> Dict:
        """更新工作流中的图片路径和提示词"""
        try:
            # 深拷贝工作流
            import copy
            ...
            if '43' in updated_workflow:
                ...
                # 添加认证参数
                updated_workflow['43']['inputs']['auth_token'] = None
                updated_workflow['43']['inputs']['comfy_api_key'] = self.comfyui_api_key
    ...
    def wait_for_completion(self, prompt_id: str, timeout: Optional[int] = None) -> Tuple[bool, Optional[Dict]]:
        """等待任务完成
        ...
        Returns:
            Tuple[bool, Optional[Dict]]: (是否成功, 结果数据)
        """
        if timeout is None:
            timeout = config.comfyui_max_wait_time
        start_time = time.time()
        
        while time.time() - start_time < timeout:
            status, result = self.client.check_status(prompt_id)
            ...
            if status == 'completed':
                ...
                logger.info(f"任务进行中: {prompt_id}, 状态: {status}")
                time.sleep(config.comfyui_check_interval)  # 使用配置中的检查间隔
            else:
    ...
    def extract_video_url(self, result_data: Dict) -> Optional[str]:
        """从结果数据中提取视频URL"""
        try:
            # 首先检查是否是Kling API的直接响应（包含video_url字段）
            if 'video_url' in result_data:
                video_url = result_data['video_url']
                logger.info(f"从Kling API响应中提取到视频URL: {video_url}")
                return video_url
            
            # 检查是否在outputs中有video_url
            outputs = result_data.get('outputs', {})
            for node_id, node_output in outputs.items():
                if 'video_url' in node_output:
                    video_url = node_output['video_url']
                    logger.info(f"从节点{node_id}输出中提取到视频URL: {video_url}")
                    return video_url
            
            # 兼容ComfyUI SaveVideo节点的输出格式
            for node_id, node_output in outputs.items():
                # 检查videos字段（旧格式）
                    ...
                    if videos and len(videos) > 0:
                        ...
                        if 'filename' in video_info:
                            ...
                            if subfolder:
                                ...
                                video_url = f"/view?filename={filename}&type=output"
                            logger.info(f"从SaveVideo节点(videos)提取到视频URL: {video_url}")
                            return video_url
                
                # 检查images字段（新格式，SaveVideo节点可能输出到images）
                if 'images' in node_output:
                    images = node_output['images']
                    if images and len(images) > 0:
                        video_info = images[0]
                        if 'filename' in video_info and video_info['filename'].endswith('.mp4'):
                            # 构建视频URL
                            filename = video_info['filename']
                            subfolder = video_info.get('subfolder', '')
                            if subfolder:
                                ...
                                video_url = f"/view?filename={filename}&type=output"
                            logger.info(f"从SaveVideo节点(images)提取到视频URL: {video_url}")
                            return video_url
            
            logger.error(f"未找到视频输出，结果数据结构: {result_data}")
            return None
    ...
    def process_single_image(self, image_path: Path, prompt: str, output_filename: str) -> bool:
        """处理单个图片生成视频"""
        try:
            ...
            if not uploaded_name:
                logger.error(f"图片上传失败: {image_path.name}")
                return False
            ...
            # 更新工作流（使用上传后的文件名）
            updated_workflow = self.update_workflow_for_image(workflow, Path(uploaded_name), prompt)
            
            # 提交任务
            prompt_id = self.client.queue_prompt(updated_workflow)
            if not prompt_id:
                return False
            
            # 等待完成
            success, result = self.wait_for_completion(prompt_id)
            if not success:
                # 记录详细的错误信息
                if result:
            ...
            if success:
                logger.info(f"图片 {image_path.name} 处理完成，视频保存为: {output_filename}")
    ...
    def get_processing_status(self, prompt_source: str = 'sd') -> Dict:
        ...
        try:
            ...
            # 检查提示词文件是否存在
            prompt_file = self.sd_prompt_file if prompt_source == 'sd' else self.flux_prompt_file
    ...
    def generate_videos(self, prompt_source: str = 'sd', start_index: Optional[int] = None, end_index: Optional[int] = None) -> bool:
        """批量生成视频
        ...
        try:
            ...
            if len(image_files) != len(prompts):
                logger.warning(f"图片数量({len(image_files)})与提示词数量({len(prompts)})不匹配")
                # 使用较小的数量
            ...
            for i, (image_path, prompt_data) in enumerate(zip(image_files, prompts)):
                try:
                    ...
                    # 处理单个图片
                    if self.process_single_image(image_path, english_prompt, output_filename):
                        success_count += 1
...
def main():
    ...
    parser.add_argument('--prompt-source', choices=['sd', 'flux'], default='sd',
    ...
    try:
        ...
        if args.status:
            ...
            print(f"  提示词文件: {'存在' if status['prompt_file_exists'] else '不存在'}")
            return True
