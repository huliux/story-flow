...
def get_prompts(path: Union[str, Path]) -> Tuple[List[str], List[Optional[str]]]:
    ...
    try:
        ...
        for item in data_list:
            ...
            # 尝试多种LoRA字段名
            lora_param = item.get("lora_id", "") or item.get("LoRA编号", "") or ""
...
def save_generation_params(
    params_file: Union[str, Path], output_file: str, data: Dict
) -> bool:
    """保存生成参数到文件
...
def main(json_file_path: Optional[str] = None) -> bool:
    ...
    for i, (prompt_b, lora_param_no) in tqdm(
        enumerate(zip(prompts, lora_param_nos)), total=total_count, desc="正在生成图片"
    ):
        # 跳过空提示词
                ...
                if "images" in response_data and response_data["images"]:
                    if save_img(response_data["images"][0], output_path):
                        ...
                        save_generation_params(
                            config.params_json_file, output_file, data
                        )
                    else:
...
def interactive_regenerate(
    url: str,
    prompts: List[str],
    lora_param_nos: List[Optional[str]],
    lora_param_dict: Dict,
    existing_files: set,
    output_dir: Path,
) -> int:
    """交互式重新生成指定图片
                ...
                for redo_img_no in redo_img_nos:
                    try:
                        ...
                        if img_index < 0 or img_index >= len(prompts):
                            tqdm.write(
                                f"图片编号 {redo_img_no} 超出范围 (1-{len(prompts)})"
                            )
                            continue
                        ...
                        # 用户可选择修改LoRA参数
                        lora_param_change = input(
                            "修改LoRA（删除输入'n'，数字加载对应配置，直接回车保持默认）："
                        )
                        if lora_param_change.lower() == "n":
                            ...
                            if lora_param_change.strip().isdigit():
                                ...
                                if lora_param:
                                    tqdm.write(
                                        f"已加载LoRA模型 {lora_model_no}: {lora_param}"
                                    )
                                else:
                            ...
                            try:
                                lora_param_no = (
                                    int(redo_lora_param_no) if redo_lora_param_no else 0
                                )
                                lora_param = lora_param_dict.get(lora_param_no, "")
                        ...
                        if response and response.status_code == 200:
                            try:
                                ...
                                if (
                                    "images" in response_json
                                    and response_json["images"]
                                ):
                                    if save_img(
                                        response_json["images"][0], output_path
                                    ):
                                        # 保存参数
                                        save_generation_params(
                                            config.params_json_file, output_file, data
                                        ...
                                        pbar.set_description(
                                            f"已重绘 {redo_count}/{len(redo_img_nos)} 张"
                                        )
                            ...
                            error_code = (
                                response.status_code if response else "连接失败"
                            )
                            tqdm.write(
                                f"❌ 重绘失败: {output_file}, 错误: {error_code}"
                            )
...
if __name__ == "__main__":
    try:
        ...
        parser.add_argument(
            "--auto", action="store_true", help="自动化模式，不进入交互式重绘"
        )
        ...
        logging.info(
            f"启动图像生成器 - 自动模式: {auto_mode}, JSON文件: {json_file or '默认'}"
        )
        ...
        if success and not auto_mode:
            try:
                ...
                target_json_file: Path = (
                    Path(json_file) if json_file else config.output_json_file
                )
                ...
                if existing_files:
                    ...
                    redo_count: int = interactive_regenerate(
                        url,
                        prompts,
                        lora_param_nos,
                        lora_param_dict,
