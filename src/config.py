...
try:
    ...
    print(
        # TODO: 手动修复长行 -         "Warning: python-dotenv not installed. Please install it using: pip install python-dotenv"
    )
...
class Config:
    ...
    def f1_default_template_uuid(self) -> str:
        """F.1默认参数模板ID"""
        return os.getenv("F1_DEFAULT_TEMPLATE_UUID", "6f7c4652458d4802969f8d089cf5b91")
    ...
    def output_dir(self) -> Path:
        """通用输出目录"""
        return self.project_root / os.getenv("OUTPUT_DIR", "data/output")
    ...
    def output_csv_file(self) -> Path:
        return self.project_root / os.getenv(
            "OUTPUT_CSV_FILE", "data/output/processed/txt.csv"
        )
    ...
    def comfyui_api_key(self) -> str:
        ...
        return os.getenv(
            "COMFYUI_API_KEY",
            "comfyui-59812650bdf5c40d3f25c39d255235d9feb41933082ac8951576993120e37cb5",
        )
    ...
    def video_load_subtitles(self) -> bool:
        ...
        return self._get_bool(
            "VIDEO_SUBTITLE", self._get_bool("VIDEO_LOAD_SUBTITLES", False)
        )
    ...
    # 视频配乐配置
    @property
    def video_music_default_file(self) -> str:
        """默认背景音乐文件名"""
        return os.getenv("VIDEO_MUSIC_DEFAULT_FILE", "aiqing.mp3")

    @property
    def video_music_volume(self) -> float:
        """背景音乐音量 (0.0-1.0)"""
        return self._get_float("VIDEO_MUSIC_VOLUME", 0.3)

    @property
    def video_music_fade_duration(self) -> float:
        """音乐淡入淡出时长（秒）"""
        return self._get_float("VIDEO_MUSIC_FADE_DURATION", 2.0)

    @property
    def video_music_original_audio_volume(self) -> float:
        """原视频音频音量 (0.0-1.0)"""
        return self._get_float("VIDEO_MUSIC_ORIGINAL_AUDIO_VOLUME", 0.7)

    @property
    def output_dir_music(self) -> Path:
        """音乐文件输出目录"""
        music_dir = self.output_dir / "music"
        self._ensure_directory_exists(music_dir)
        return music_dir
    ...
    def validate_config(self) -> List[str]:
        ...
        if self.llm_provider == "openai":
            ...
            errors.append(
                f"Unsupported LLM provider: {self.llm_provider} (supported: openai, deepseek)"
            )
