...
class ViralVideoGenerator:
    ...
    def get_user_input(self) -> Optional[Tuple[str, str, int]]:
        ...
        while True:
            try:
                scene_count_input = input("请输入场景数量（默认5个，直接回车使用默认值）: ").strip()
    ...
    def generate_video_outline(self, video_theme: str, style_reference: str, scene_count: int) -> Optional[Dict]:
        """
        生成视频大纲
...
"""
        ...
        try:
            ...
            try:
                ...
                if json_start != -1 and json_end != -1:
                    ...
                    # 验证必需字段
                    required_fields = ['video_description_zh', 'video_description_en', 'suitable_scene_count']
    ...
    def _generate_fallback_outline(self, video_theme: str, scene_count: int) -> Dict:
        ...
        # 简单的英文翻译（基础版本）
        theme_en = video_theme.replace('男女', 'couple').replace('相爱', 'love').replace('结婚', 'marriage').replace('相伴', 'companionship').replace('时间流逝', 'passage of time').replace('伤感', 'melancholy')
    ...
    def generate_flux_prompts(self, outline_data: Dict, trigger_word: str = "", special_requirements: str = "") -> Optional[Dict]:
        """
        生成Flux1格式的生图提示词
...
4. 请控制每条提示词的字数在 250-350 字范围内，确保完全符合 Flux 图像生成模型的输入要求和最佳实践。
"""
...
请为每个场景生成详细的中英文提示词，确保提示词具有视觉冲击力和爆款潜质。
严格按照JSON格式输出。
"""
    ...
    def save_outline_to_input(self, outline_data: Dict, video_theme: str) -> bool:
        ...
        try:
            ...
            safe_theme = "".join(c for c in video_theme if c.isalnum() or c in (' ', '-', '_')).strip()
    ...
    def generate_complete_workflow(self) -> bool:
        ...
        try:
            ...
            # 第二步：生成视频大纲
            outline_data = self.generate_video_outline(video_theme, style_reference, scene_count)
